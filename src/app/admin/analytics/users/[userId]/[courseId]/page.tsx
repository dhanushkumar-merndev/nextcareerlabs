...
export default async function UserCourseDetailedAnalyticsPage({
  params,
}: PageProps) {
                    ...
                    <div key={lesson.id} className="group/lesson">
                      <div className="flex flex-row items-start py-4 px-2 rounded-xl hover:bg-muted/30 transition-all duration-300">
                        <div className="flex items-start gap-4 flex-1 min-w-0">
                          <div
                            className={`flex items-center justify-center size-10 rounded-full border shrink-0 transition-colors ${isCompleted ? "bg-primary/10 border-primary/20 text-primary" : "bg-muted/10 border-border/50 text-muted-foreground/30"}`}
                          >
                            {isCompleted ? (
                            ...
                            <div className="flex flex-col gap-1.5 mt-2">
                                ...
                                <div className="flex flex-col gap-1.5">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] font-black text-foreground uppercase tracking-widest leading-none">
                                      {formatTime(progress.lastWatched)}
                                    </span>
                                  </div>
                                  <div className="flex items-center gap-2">
                              ...
                              <div className="mt-3">
                                {isCompleted ? (
                                  ...
                                  <Badge
                                    variant="outline"
                                    className="border-primary/40 text-primary bg-primary/5 rounded-full px-5 py-1 text-[10px] font-black uppercase tracking-widest min-w-[85px] justify-center w-fit"
                                  >
                                    ACTIVE
                                  </Badge>
                                ) : (
                                  <Badge
                                    variant="outline"
                                    className="border-border text-muted-foreground/20 bg-muted/5 rounded-full px-5 py-1 text-[10px] font-black uppercase tracking-widest min-w-[85px] justify-center opacity-40 w-fit"
                                  >
                                    PENDING
                                  </Badge>
                                )}
