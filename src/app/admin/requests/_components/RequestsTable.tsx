...
export function RequestsTable({
  ...
  const clearLocalCache = useCallback(
    (userId?: string) => {
      ...
      if (userId) {
        secureStorage.removeItemTracked(
          `chat_cache_${userId}_user_enrolled_courses_${userId}`,
        );
        secureStorage.removeItemTracked(
          `chat_cache_${userId}_available_courses_${userId}`,
        );
        secureStorage.removeItemTracked(
          `chat_cache_${userId}_user_dashboard_${userId}`,
        );
      }
      ...
      console.log(
        `[RequestsTable] Local + React Query caches cleared ${userId ? "including user pages " : ""}after admin action.`,
      );
    },
    [queryClient],
  );
        ...
        startTransition(async () => {
          ...
          if (result.status === "success") {
            ...
            setData((prev) =>
              prev.map((item) =>
                item.User.id === userId
                  ? { ...item, User: { ...item.User, banned: !isBanned } }
                  : item,
              ),
            );
          } else {
  ...
  const handleDelete = (id: string) => {
    setConfirmConfig({
      ...
      description:
        "This action cannot be undone. This will permanently delete the enrollment request from the database.",
  ...
  useEffect(() => {
    ...
    startTransition(async () => {
      ...
      setData(searchData);
      setTotalCount(result.totalCount);
      setHasMore(searchData.length < result.totalCount);
      console.log(
        `[RequestsTable] Search synced with server for "${debouncedSearch}".`,
      );
    });
  }, [debouncedSearch]);
  ...
  return (
    <div className="space-y-4 sm:space-y-6 px-4 sm:px-0">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          >
            <RefreshCw
              className={cn(
                "size-4",
                (refreshing || isPending) && "animate-spin text-primary",
              )}
            />
          </Button>
        </div>
                ...
                >
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm font-medium">
                      No enrollment requests found matching filters.
                    </p>
                  </div>
                </TableCell>
              </TableRow>
              ...
              data.map((request) => (
                ...
                >
                      ...
                      <div className="flex flex-col gap-0.5">
                        <div className="flex items-center gap-2">
                          ...
                          {request.User.banned && (
                            <Badge
                              variant="destructive"
                              className="h-4 px-1 text-[8px] uppercase tracking-tighter font-black"
                            >
                              Banned
                            </Badge>
                          )}
                        ...
                        <button
                          onClick={() =>
                            copyToClipboard(request.User.id, "User ID")
                          }
                  ...
                  <TableCell className="text-center">
                    <button
                      onClick={() =>
                        copyToClipboard(request.User.email, "Email")
                      }
                  ...
                  <TableCell className="text-center">
                    {request.User.phoneNumber ? (
                      <button
                        onClick={() =>
                          copyToClipboard(request.User.phoneNumber!, "Phone")
                        }
                        className="inline-flex items-center gap-1.5 text-xs text-foreground/70 hover:text-primary transition-colors font-medium cursor-pointer"
                      >
                      ...
                      <span className="text-[10px] text-muted-foreground/40 italic uppercase tracking-tighter">
                        None
                      </span>
                    )}
                  </TableCell>
      ...
      <div className="grid grid-cols-1 gap-4 lg:hidden">
        {data.length === 0 ? (
          ...
          data.map((request) => (
            <div
              key={request.id}
              className="bg-card/40 backdrop-blur-md border border-border/40 rounded-3xl p-5 shadow-xl space-y-5 relative overflow-hidden group transition-all"
            >
              {/* Status Ribbon/Badge */}
                ...
                <div className="flex flex-col justify-center">
                  <h3 className="font-black text-sm uppercase tracking-tight flex items-center gap-2">
                    ...
                    {request.User.banned && (
                      <Badge
                        variant="destructive"
                        className="h-3 px-1 text-[7px] font-black"
                      >
                        BANNED
                      </Badge>
                    )}
                  ...
                  >
                    <Fingerprint className="size-2.5" /> ID:{" "}
                    {request.User.id.substring(0, 8)}...
                  </button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3 border-y border-border/20 py-5">
                <DetailRow
                  ...
                  onCopy={() => copyToClipboard(request.User.email, "Email")}
                />
                <DetailRow
                  icon={Phone}
                  label="Phone"
                ...
                <DetailRow
                  icon={BookOpen}
                  label="Course"
                  value={request.Course.title}
                />
                <DetailRow
                  icon={CalendarIcon}
                  label="Requested"
                  value={formatIST(request.createdAt)}
                />
                <DetailRow
                  icon={CalendarIcon}
                  label="Joined"
                  value={formatIST(request.User.createdAt)}
                />
              </div>
        ...
        <DialogContent className="sm:max-w-md border-2 border-border/40 bg-card/95 backdrop-blur-xl rounded-3xl p-6">
          ...
          <DialogFooter className="gap-2 sm:gap-0 pt-2">
            <Button
              ...
              className="font-bold uppercase tracking-widest text-[10px] h-11 rounded-xl"
            >
              Cancel
            </Button>
            <Button
              ...
              disabled={isPending}
              className="font-bold uppercase tracking-widest text-[10px] h-11 px-8 rounded-xl shadow-lg shadow-primary/20 transition-all hover:scale-[1.02] active:scale-[0.98]"
            >
              {isPending ? (
      ...
      <AlertDialog
        ...
        onOpenChange={(open) =>
          !open && setConfirmConfig((prev) => ({ ...prev, open }))
        }
      >
        <AlertDialogContent className="border-2 border-border/40 bg-card/95 backdrop-blur-xl rounded-3xl p-6">
          ...
          <AlertDialogFooter className="gap-2">
            <AlertDialogCancel className="font-bold uppercase tracking-widest text-[10px] h-11 rounded-xl">
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={(e) => {
  ...
  return (
    <div className="flex items-start justify-between text-[11px]">
      ...
      <button
        ...
        onClick={onCopy}
        className={cn(
          "font-bold text-foreground text-right tracking-tight truncate ml-4",
          onCopy &&
            "active:text-primary transition-all flex flex-row-reverse items-center gap-1.5",
        )}
      >
        {value}
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          className="h-10 w-10 p-0 hover:bg-muted/80 rounded-full transition-colors"
          disabled={isPending}
        >
      ...
      <DropdownMenuContent
        align="end"
        className="w-56 p-2 rounded-2xl border-2 border-border/40 bg-card/95 backdrop-blur-xl shadow-2xl"
      >
        <DropdownMenuLabel className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 p-3">
          Manage Profile
        </DropdownMenuLabel>

        <DropdownMenuItem
          onClick={() => onEditOpen(request.User)}
          className="rounded-xl p-3 focus:bg-primary/5 group cursor-pointer border border-transparent focus:border-primary/10 transition-all mb-1"
        >
        ...
        {request.status !== "Granted" && (
          <DropdownMenuItem
            onClick={() => onStatusUpdate(request.id, "Granted")}
            className="rounded-xl p-3 focus:bg-emerald-500/5 group cursor-pointer border border-transparent focus:border-emerald-500/10 transition-all mb-1"
          >
        ...
        {request.status !== "Revoked" && (
          <DropdownMenuItem
            onClick={() => onStatusUpdate(request.id, "Revoked")}
            className="rounded-xl p-3 focus:bg-rose-500/5 group cursor-pointer border border-transparent focus:border-rose-500/10 transition-all mb-1"
          >
        ...
        {request.User.banned ? (
          <DropdownMenuItem
            onClick={() => onBanToggle(request.User.id, true)}
            className="rounded-xl p-3 focus:bg-blue-500/5 group cursor-pointer border border-transparent focus:border-blue-500/10 transition-all"
          >
          ...
          <DropdownMenuItem
            onClick={() => onBanToggle(request.User.id, false)}
            className="rounded-xl p-3 text-destructive focus:bg-destructive/5 group cursor-pointer border border-transparent focus:border-destructive/10 transition-all"
          >
