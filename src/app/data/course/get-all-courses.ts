...
const getAllCoursesInternal = async (
  ...
  const [coursesVersion, userVersion] = await Promise.all([
    ...
    userId ? getGlobalVersion(GLOBAL_CACHE_KEYS.USER_VERSION(userId)) : Promise.resolve("")
  ]);

  const currentVersion = userId ? `${coursesVersion}:${userVersion}` : coursesVersion;
  ...
  if (searchQuery) {
    ...
    const searchRaw = await prisma.course.findMany({
      ...
      select: {
        id: true, title: true, smallDescription: true, duration: true,
        level: true, fileKey: true, category: true, slug: true,
        chapter: {
          orderBy: { position: "asc" }, take: 1,
          select: { lesson: { orderBy: { position: "asc" }, take: 1, select: { id: true } } }
        }
      },
    });
    allCourses = searchRaw.map(c => ({
      id: c.id, title: c.title, smallDescription: c.smallDescription,
      duration: c.duration, level: c.level, fileKey: c.fileKey,
    ...
    await setCache(cacheKey, { data: allCourses, version: currentVersion }, 2592000);
  ...
  if (userId) {
    ...
    if (onlyAvailable) {
      ...
      resultCourses = page.map((c) => ({ ...c, enrollmentStatus: map.get(c.id) ?? null }));
      ...
      return { status: "data", version: currentVersion, courses: resultCourses, nextCursor };
    }
  }
