...
import {
  ...
  deleteMessageAction,
  editMessageAction,
  banUserFromSupportAction,
  archiveThreadAction,
  getGroupParticipantsAction,
...
export function ChatWindow({
  ...
  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      ...
      if (e.key?.includes(cacheKey)) {
        console.log(
          `[ChatWindow] Participants updated in another tab (Storage: ${e.key}). Refetching...`,
        );
        // Clear the metadata sync check (so it doesn't think it's fresh)
        secureStorage.removeItemTracked(
          `chat_cache_${currentUserId}_${cacheKey}_sync`,
        );
  ...
  const handleSendMessage = async () => {
    ...
    (async () => {
      try {
        let result;
        if (isAdmin && !isGroup) {
          let recipientId = messages.find(
            (m) => m.type === "SUPPORT_TICKET",
          )?.senderId;

          if (!recipientId) {
            recipientId = messages.find(
              ...
              page.messages?.map((n: any) =>
                n.id === id
                  ? {
                      ...n,
                      resolved: isThreadResolved,
                      feedback: finalizedFeedback || n.feedback,
                    }
                  : n,
              ) || page.messages,
          })),
  ...
  const submitEditDialog = async () => {
    if (
      !editingMessageId ||
      (!editContent.trim() && !editImageUrl && !editFileUrl)
    )
      return;
      ...
      window.dispatchEvent(
        new CustomEvent("chat-thread-update", {
          detail: {
            ...
            lastMessage:
              newContent ||
              (newImageUrl ? "Image" : newFileUrl ? "PDF" : "Message"),
            updatedAt: new Date().toISOString(),
          },
        }),
      );
      ...
      if (lastMsg) {
        ...
        if (!preview.trim()) {
          if (lastMsg.imageUrl) preview = "Image";
            ...
            preview = `PDF (${lastMsg.fileName || "Document"})`;
        }
  ...
  const handleShowGroupInfo = async () => {
    ...
    if (cached) {
      ...
      const lastSyncStr = secureStorage.getItem(
        `chat_cache_${currentUserId}_participants_${threadId}_sync`,
      );
      ...
      if (now - lastSync < STALE_TIME) {
        isStale = false;
        console.log(
          `[ChatWindow] Group participants are fresh. Skipping Redis request.`,
        );
      }
    }
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-muted/20 overflow-hidden">
                ...
                <>
                  ...
                  >
                    ...
                    <span className="font-medium">
                      {isBanned ? "Unban Ticket Access" : "Ban Ticket Access"}
                    </span>
                  </DropdownMenuItem>
                </>
              )}
      ...
      {showGroupInfo ? (
        ...
        <div
          data-lenis-prevent
          className="flex-1 min-h-0 overflow-y-auto overscroll-contain no-scrollbar bg-background"
        >
          <div className="p-6 space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="space-y-2 px-1">
              ...
              <p className="text-sm border rounded-xl p-3 bg-muted/20">
                This is the official group for <strong>{title}</strong>. Stay
                tuned for all important updates, resources, and announcements.
              </p>
            </div>
              ...
              <div className="space-y-1">
                {groupParticipants.map((p, i) => (
                  ...
                  >
                    ...
                    {p.role === "admin" && (
                      <span className="text-[9px] font-bold bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full border border-blue-200">
                        ADMIN
                      </span>
                    )}
                  </div>
            ...
            {loading ? (
              <div className="space-y-6 p-4">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className={`flex gap-3 max-w-[80%] ${i % 2 === 0 ? "ml-auto flex-row-reverse" : ""}`}
                  >
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isMe
                        ? "ml-auto items-end md:items-start md:flex-row-reverse"
                        : "items-start",
                    )}
                  >
                    {showAvatar ? (
                              ...
                              <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                  <Button
                                    ...
                                    className="h-5 w-5 rounded-full hover:bg-black/10 p-0"
                                  >
                                    <ChevronDown
                                      className={cn(
                                        "h-3 w-3",
                              ...
                              <div className="space-y-3">
                                ...
                                <div className="relative">
                                  ...
                                  <span
                                    className={cn(
                                      ...
                                      resolveFeedbackText.length >= 280
                                        ? "text-orange-500"
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    handleResolve(
                                      msg.id,
                                      resolveFeedbackText || resolveStatus!,
                                    )
                                  }
                                >
                                  {resolveStatus === "Helpful"
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span>
                                {(() => {
                                  ...
                                  if (
                                    f.includes("positive feedback") ||
                                    ["helpful", "yes"].includes(f)
                                  )
                        ...
                        msg.type === "SUPPORT_TICKET" && (
                          <div className="flex gap-2 mt-1 px-1">
                            <Button
                              ...
                              onClick={() =>
                                handleResolveTicket(msg.id, "Resolved")
                              }
                            >
                            ...
                            <Button
                              ...
                              onClick={() =>
                                handleResolveTicket(msg.id, "Denied")
                              }
                            >
            ...
            <div className="p-4 bg-background border-t shrink-0">
              {imageUrl && (
                <div className="flex items-center gap-2 mb-2 p-2 bg-muted rounded-lg w-fit">
                  ...
                  <button
                    onClick={() => handleRemoveAttachment(imageUrl, "image")}
                  >
                    <X className="h-4 w-4" />
                  </button>
                </div>
              ...
              {fileUrl && (
                <div className="flex items-center gap-2 mb-2 p-2 bg-muted rounded-lg w-fit">
                  ...
                  <button
                    onClick={() => handleRemoveAttachment(fileUrl, "file")}
                  >
                    <X className="h-4 w-4" />
                  </button>
                </div>
              ...
              <div className="flex items-end gap-2 bg-muted/30 p-2 rounded-xl border focus-within:ring-1 focus-within:ring-primary/20 transition-all">
                ...
                <Button
                  ...
                  disabled={
                    (!inputText.trim() && !imageUrl && !fileUrl) || isUploading
                  }
            ...
            <div className="relative mb-2 w-fit">
              <img
                src={
                  ...
                  editImageUrl.startsWith("/")
                    ? editImageUrl
                    : constructUrl(editImageUrl)
                }
                alt="edit attachment"
